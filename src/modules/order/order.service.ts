...
export class OrderService {
	...
	async createOneWithPaymentProduct2(request: CRequest, body: OrderCreateOneWithPaymentProductRequest) {
		const client = await this.partnerService.getOne({ id: body.clientId })
		const clientRole = client.data.roles.find((r) => r.name === PartnerRoleEnum.client)
		if (!clientRole) {
			throw new BadRequestException('client not found')
		...
		await this.googleSheetService.addOrderToSheet(order, [...orderProducts.data, ...mappedOrSPs], order.payments)
