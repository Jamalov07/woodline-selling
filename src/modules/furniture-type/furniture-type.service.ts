...
export class FurnitureTypeService {
	...
	async createOne(body: FurnitureTypeCreateOneRequest) {
		...
		if (candidate) {
			return createResponse({ data: candidate, success: { messages: ['create success'] }, warning: { is: true, messages: ['name already exists'] } })
		}

		const furnitureType = await this.furnitureTypeRepository.createOne({ ...body })

		return createResponse({ data: furnitureType, success: { messages: ['create success'] } })
	}
	...
	async updateOne(query: FurnitureTypeGetOneRequest, body: FurnitureTypeUpdateOneRequest) {
		...
		const furnitureType = await this.furnitureTypeRepository.updateOne(query, { ...body })

		return createResponse({ data: furnitureType, success: { messages: ['update one success'] } })
	}
